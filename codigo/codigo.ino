...
const char* ssid = "ZTE_2.4G_qpmQHZ";
const char* password = "chGdHUzy"; 

//Servidor MQTT
const char* mqtt_server = "abvr356rxgubn-ats.iot.us-east-1.amazonaws.com";
...
void setup() {
  ...
  //*****************************
  // Cert leer archivo
  File file4 = SPIFFS.open("/beeea2-certificate.pem.crt", "r");
  if (!file4) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
  ...
  File file6 = SPIFFS.open("/beeea2-private.pem.key", "r");
  if (!file6) {
    Serial.println("No se pudo abrir el archivo para leerlo");
    return;
  }
...
void loop() {
  ...
  if (now - lastMsg > 5000) {
    ...
    //snprintf (msg, BUFFER_LEN, "{\"mac_Id\" : \"%s\", \"temperature\" : %s, \"humidity\" : %s}", macIdStr.c_str(), Temprature.c_str(), Humidity.c_str());
...
/*
Introducción, objetivos, problemática, Alcances 


Desarrollo1


Desarrollo2  Conclusiones
